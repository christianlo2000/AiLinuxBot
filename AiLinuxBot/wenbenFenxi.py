...
class Texto:
    ...
    def __init__(self,texto):
        self.__texto = texto
        self.__analisisSin = self.analisisTexto()
        self.__cantPal = len(self.__analisisSin)
    def getTexto(self):
        return self.__texto
    def getAnalisisSin(self):
        return self.__analisisSin
    def getCantPal(self):
        return self.__cantPal
    # similar a la palabra ingresada
    def similar(self,palabra):
        # validamos si la palabra existe
        pal = []
        fun = []
        pal,fun = zip(*self.getPalFuncText(palabra))
        print("fun: "+str(fun))
        if (not pal):
            # no existe, tiramos mensaje error
            print("Palabra "+palabra+"NO existe en texto")
            return False
        # barremos lista para generar lista salida de similares
        listSal = []
        listPal,listFunc = zip(*self.__analisisSin)
        i = 0
        for pal1 in listPal:
            if (listFunc[i] == fun):         
                  print("encontre similar")
                  listSal.append(pal1)
                  i = i + 1
        return listSal
        ...
        #pass
    # buscamos palabra y funcion sintactica
    def getPalFuncText(self,palabra):
        palList,palFunc = zip(*self.__analisisSin)
        palSal = []
        i = 0
        for pal in palList:
            if (pal == palabra):
                #print("encontre palabra")
                palSal = zip(palList[i],palFunc[i])
                return palSal
            i = i + 1
        palSal = (False,False)   
        return palSal
    ...
    # funciones de analisis de palabras
	...
	# es singular
    	return palabra
    # determinamos genero de la palabra
    def genero(self,palabra):
	if (self.esMasculino(palabra) == True and self.esFemenino(palabra) == True):
		return "Ambos generos"
	elif (self.esMasculino(palabra) and not self.esFemenino(palabra)):
		return "Masculino"
	elif (not self.esMasculino(palabra) and self.esFemenino(palabra)):
		return "Femenino"
	elif (palabra.endswith("uz")):
		return "Femenino"
	else:
		return "Genero Indeterminado"
    def esMasculino(self,palabra):
	if (palabra.endswith("o") or palabra.endswith("os")):
		return True
	# analisis de excepciones
	if (palabra.endswith("e") or palabra.endswith("es")):
		return True
        if (palabra.lower() in ["mano"]):
                return False
        # excepciones terminadas en a
	if (palabra[0].lower() == "a" and palabra.endswith("a") and palabra.lower() != "angustia"):
                # ej: agua --> es masculino
                return True
        if (palabra.lower() in ["día","maná","papá"]):
                return True
        if (palabra.endswith("igma") or palabra.endswith("igmas")):
                return True

	if (palabra.endswith("ema") or palabra.endswith("emas")):
		return True
	return False
    def esFemenino(self,palabra):
     	# pregunto si es masculino
     	if (self.esMasculino(palabra)):
     		return False
     	if (palabra.endswith("a")):
		return True
	if (palabra.endswith("as")):
		return True
	if (palabra.endswith("ión") or palabra.endswith("iones")):
		return True
	return False
    # verificamos si es sustantivo (no 100% seguro)
    def esSustantivo(self,palabra):
    	if (palabra.endswith("r") and not self.esVerboInfinitivo(palabra)):
		return True
	if (palabra.endswith("ion") or palabra.endswith("iones")or palabra.endswith("ión")):
		return True
	if (palabra.endswith("terio") or palabra.endswith("torio") or palabra.endswith("bilidad") or palabra.endswith("tero") or palabra.endswith("tera") or palabra.endswith("ólogo") or palabra.endswith("óloga") or palabra.endswith("copio")):
		return True
	return False
    # verifico si es verbo infinitivo
    def esVerboInfinitivo(self,palabra):
	# validamos excepciones
	if (palabra in ["ayer","caracter","cráter"]):
		return False
	if (palabra.endswith("ar") or palabra.endswith("er") or palabra.endswith("ir")):
		return True
	return False
    # verificamos si es adjetivo
    def esAdjetivo(self,palabra):
	# abro archivo para grabar
	archAdj = open("Adjetivos.txt",'r')
	if (palabra.endswith("ivo") or palabra.endswith("ivos") or palabra.endswith("iva") or palabra.endswith("ivas") or (len(palabra) > 3 and palabra.endswith("az"))):
		return True
	# verifico si esta en el archivo
	for pal in archAdj:
		#print("palabra adjetivo: "+pal+"|")
		# validamos si pal existe en dir
		if (pal.rstrip('\r\n') == palabra.lower()):
			archAdj.close()	
			#print("Encontre adjetivo en dir: "+pal)
			return True
	archAdj.close()
	return False
    # verificar si es verbo copulativo: ser, estar, parecer.
    def esVerboCopulativo(self,palabra):
	if (palabra.lower() in ["es","son","fue","fueron","será","serán","está","están","estuvo","estuvieron","estarán","parece","parecen","parecía","parecían","parecerá","parecerán"]):
		return True
	return False	
    # grabamos palabra por valor morfologico
    def grabarPalabra(self,palabra,archivo):
	# abrimos archivo 
	archPal = open(archivo,'a')
	# verificamos si ya existe la palabra
	if (self.existePalabra(palabra,archivo)):
		archPal.close()
		return True
	#archPal = open(archivo,'a')
	# falta validar qeu existe o no palabra
	archPal.write(palabra)
	archPal.write('\n')
	archPal.close() 
	return True
    def existePalabra(self,palabra,archivo):
	archPal = open(archivo,'r')
	registro = archPal.readlines()
	for pal in registro:
	...
	archPal.close()
	return False
    # grabamos log de oracion analizada
    def grabarArchivoLog(self,palabra,archivo):
        archPal = open(archivo,'a')
        # falta validar qeu existe o no palabra
        archPal.write(palabra)
	archPal.write('\n')
    ...
    # validamos si es o no verbo
    def esVerbo(self,palabra):
		# valido si termina con "r", que si bien es verbo,
		# no cumple funcion de verbo
		if (palabra.endswith("r")):
			return False
		# valido si es verbo copulativo
		if (self.esVerboCopulativo(palabra)):
			return True
		# validamos si es verbo viendo si es gerundio
		if (palabra.endswith("ando") or palabra.endswith("endo")):
			return True
		# validamos si es adjetivo o sustantivo
		if (self.esSustantivo(palabra) or self.esAdjetivo(palabra)):
			return False
		# antes de concatenar debemos cambiar las vocales
		# acentuadas por vocales sin acento
		...
		palabra = re.sub("ú","u",palabra)	
        	# concatenamos el verbo con la url del diccionario
        	dic = "https://conjugador.reverso.net/conjugacion-espanol"+"-verbo-"+palabra+".html"
        	#dicPage = urlopen(dic)
		texto=""	
		try:
        		dicPage = urllib2.urlopen(dic)	
        		# leemos url
        		texto = str(dicPage.read())
		except:
			print("Error conexion web de conjugacion")
			texto = "ERROR"
        	#print(str(texto))
        	# valido si es o no verbo
        	if texto.find("verbo buscado") > -1:	
        		   dicPage.close()
        		   return False
        	if (self.esPreposicion(palabra)):
        	    dicPage.close()
        	    return False
        	# pregunto si es adverbio de cantidad
        	if (self.esAdvCant(palabra)):
		           dicPage.close()
		           return False
        	dicPage.close()
		# al ser verbo, lo grabamos
		#self.grabarPalabra(palabra.lower(),"verbo.txt")
        	return True
    ...
    def esAdvModo(self,palabra):
		...
		#self.grabarPalabra(palabra.lower(),"adverbioModo.txt")
                return True
            return False
